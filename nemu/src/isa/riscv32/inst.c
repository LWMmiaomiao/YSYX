...
#define immB() do { \
  uint64_t imm_B =  BITSLS(BITS(i,11,8), 1) | \
                    BITSLS(BITS(i,30,25), 5) | \
                    BITSLS(BITS(i,7,7), 11) | \
                    BITSLS(BITS(i,31,31), 12); \
	*imm = SEXT(imm_B, 13); \
} while(0)
#define immJ() do { \
  uint64_t imm_J =  BITSLS(BITS(i,30,21), 1) | \
                    BITSLS(BITS(i,20,20), 11) | \
                    BITSLS(BITS(i,19,12), 12) | \
                    BITSLS(BITS(i,31,31), 20); \
	*imm = SEXT(imm_J, 21); \
} while(0)
...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *rd     = BITS(i, 11, 7);
  ...
  //src1R()和src2R(), 用于寄存器的读取结果记录到相应的操作数变量中
}
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();
// [TODO]:TEST string hello-str requires the implementation of additional content to run
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1 + imm);
	INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(rd) = (sword_t)src1 < (sword_t)imm);
	INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = src1 < imm);
	INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(rd) = src1 ^ imm);
	INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(rd) = src1 & imm);
	INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, if(!(imm & 0b100000)) R(rd) = src1 << BITS(imm, 4, 0));
	INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, if(!(imm & 0b100000)) R(rd) = (word_t)src1 >> BITS(imm, 4, 0));
	INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, if(!(imm & 0b100000)) R(rd) = (sword_t)src1 >> BITS(imm, 4, 0));
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(Mr(src1 + imm, 1),8));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, 2),16));
	INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = SEXT(Mr(src1 + imm, 4),32));
	INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));
	INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + imm, 2));
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr   , I, s->dnpc = (src1 + imm) & ~1; R(rd) = s->snpc; IFDEF(CONFIG_FTRACE, FTRACE_JALR));
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add	  , R, R(rd) = src1 + src2);
	...
	INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = ((sdword_t)(sword_t)src1 * (sdword_t)(sword_t)src2) >> 32);
...
int isa_exec_once(Decode *s) {
  ...
  IFDEF(CONFIG_ITRACE, trace_iringbuf(s->pc, s->isa.inst));
