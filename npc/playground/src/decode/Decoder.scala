...
case class DecodeBase(
	...
	def generate: List[UInt] = List(rs1From, rs2From, exType, immType, aluType, lsLength, loadSignExt)
}
...
object RV32IDecode {
    ...
    // 每个元素是一个元组（Tuple），元组的第一项是指令模式（BitPat），第二项是通过`DecodeBase`生成的一系列控制信号（List[UInt]）
